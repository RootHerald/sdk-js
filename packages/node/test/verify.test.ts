...
describe('verifyAttestationToken', () => {
  it('happy path: verifies a composite EAT JWT and maps claims to AttestationVerdict', async () => {
    ...
    // Deprecated backward-compat mirrors still populated
  ...
  it('throws RootHeraldError JWT_INVALID for a tampered signature', async () => {
    const token = await fixtures.signToken({
      iss: 'https://rootherald.example.com',
      aud: 'my-client',
      sub: 'user-uuid-1',
    ...
    expect((err as RootHeraldError).code).toBe('JWT_INVALID');
  });

  it('throws RootHeraldError JWT_INVALID when acr claim is missing', async () => {
    const now = Math.floor(Date.now() / 1000);
  ...
  it('throws RootHeraldError JWT_INVALID when amr claim is missing', async () => {
    const now = Math.floor(Date.now() / 1000);
    const token = await new SignJWT({
      iss: 'https://rootherald.example.com',
      aud: 'my-client',
  ...
  it('throws RootHeraldError SCHEMA_VIOLATION for a wrong eat_profile', async () => {
    const now = Math.floor(Date.now() / 1000);
    const token = await new SignJWT({
      iss: 'https://rootherald.example.com',
      aud: 'my-client',
    ...
    expect(err).toBeInstanceOf(RootHeraldError);
    expect((err as RootHeraldError).code).toBe('SCHEMA_VIOLATION');
  });

  it('throws RootHeraldError SCHEMA_VIOLATION when ueid is absent in both container and legacy', async () => {
    const now = Math.floor(Date.now() / 1000);
    const token = await new SignJWT({
      iss: 'https://rootherald.example.com',
      aud: 'my-client',
  ...
  it('throws for a wrong audience', async () => {
    ...
    ).rejects.toThrow(RootHeraldError);
  });
});
